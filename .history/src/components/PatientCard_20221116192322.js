    ...
    return (
        <div className={styles['patient-card']}>
            <div className={styles['inner-patient-card']}>
                ...
                {patient?.stage === 'treating' && <button s onClick={() => onStartDischarge(patient)}>Start Discharge Process</button>}

                {patient?.stage === 'discharging' && <button s onClick={() => onCompleteDischarge(patient)}>Discharge Process Complete</button>}
