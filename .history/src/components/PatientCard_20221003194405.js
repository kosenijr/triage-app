...
const PatientCard = ({ patient, onStartDischarge, onCompleteDischarge, updateRoomAssignment }) => {
    ...
    return (
        <div className={styles['patient-card']}>
            ...
            <div>
                <h4>Priority</h4>
                <span className={priority === 1 ? 'font-bold' : ''} onClick>1</span>
                <span className={priority === 2 ? 'font-bold' : ''} onClick>2</span>
                <span className={priority === 3 ? 'font-bold' : ''} onClick>3</span>
            </div>
