...
const PatientCard = ({ patient, onStartDischarge, onCompleteDischarge, updateRoomAssignment }) => {
    ...
    log(patient.name, typeof (patient.complaint), patient.complaint.split(/).length);
    ...
    return (
        <div className={styles['patient-card']}>
            <div className={styles['inner-patient-card']}>
                ...
                {patient?.stage === 'treating' && <button className={styles["start-button"]} onClick={() => onStartDischarge(patient)}>Start Discharge Process</button>}

                {patient?.stage === 'discharging' && <button className={styles["complete-button"]} onClick={() => onCompleteDischarge(patient)}>Discharge Process Complete</button>}
