...
function App() {
  ...
  const onCompleteDischarge = (patient, event) => {
    patientRecords = setPatientRecords(patientRecords.map(record => record.id === patient.id ? patientRecords.splice(patientRecords.indexOf(patient), 0) : null));
  }
  return (
